...
if ! (kubectl get secret -n openfaas-fn minio-access-key || \
	...
	prompt_error "failed to assign minio access key"
	exit 1
fi
if ! (kubectl get secret -n openfaas-fn minio-secret-key || \
	...
	prompt_error "failed to assign minio secret key"
	exit 1
fi
